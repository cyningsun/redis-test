...
func main() {
	...
	lastBytes := make([]byte, RDB_EOF_MARK_SIZE)
	...
	for payload > uint64(0) {
		...
		nread, err := rw.Read(buf[:targetBytes])
		if err != nil {
		...
		payload -= uint64(nread)
		if useMark {
			if nread >= RDB_EOF_MARK_SIZE {
				copy(lastBytes, buf[nread-RDB_EOF_MARK_SIZE:])
			} else {
				rem := RDB_EOF_MARK_SIZE - nread
				copy(lastBytes, lastBytes[rem:])
				copy(lastBytes[rem:], buf[:nread])
			}
			if bytes.Equal(lastBytes, bufEOF) {
	...
	if useMark {
		payload = math.MaxUint64 - payload - RDB_EOF_MARK_SIZE
		syscall.Ftruncate(int(rdb.Fd()), int64(payload))
